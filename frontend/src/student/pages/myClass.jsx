...
const BookSession = () => {
  ...
  const [activeDate, setActiveDate] = useState(null); // null = show all days
  ...
  const tutors = useMemo(() => {
    const selectedDay = activeDate !== null ? dates[activeDate]?.day : null; // null = show all

    return classes
      .filter((cls) => {
        // Filter out classes with no schedule or no tutor
        if (!cls.schedule || cls.schedule.length === 0 || !cls.tutorId) return false;

        // If no day selected, show all classes
        if (!selectedDay) return true;

        // If day selected, only show classes with schedules for that day
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        console.log("Student classes response:", res);
        ...
        console.log("Classes set to state:", Array.isArray(data) ? data : []);
      } catch (err) {
        console.error("Error fetching classes:", err);
        console.error("Error response:", err.response?.data);
        console.error("Error status:", err.response?.status);
        setError(err.response?.data?.message || "Failed to load available classes");
      } finally {
        setLoading(false);
      }
    };
